...
/** Build callback URL so after Google OAuth we return to this page with MCP params preserved */
function useMcpCallbackUrl() {
...
function MCPAuthContent() {
  ...
  /** After Google OAuth, redirect back here so we can send the token to MCP */
  const mcpCallbackUrl = useMcpCallbackUrl();
    ...
    <div className="flex min-h-screen items-center justify-center ide-surface px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border ide-border ide-surface-panel p-6">
          {/* Google sign-in: redirects back to this page with same MCP params so we can send token to MCP */}
          {mcpCallbackUrl && (
            <>
              <a
                href={`/api/auth/login?callbackUrl=${encodeURIComponent(mcpCallbackUrl)}`}
                className="flex w-full items-center justify-center gap-3 rounded-lg border ide-border ide-surface-panel px-6 py-3 text-sm font-medium ide-text transition-all duration-150 ide-hover focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-[oklch(0.145_0_0)] dark:focus:ring-offset-[oklch(0.145_0_0)]"
              ...
              <div className="my-4 flex items-center gap-3">
                ...
                <span className="text-xs ide-text-quiet">or</span>
                <div className="h-px flex-1 bg-stone-200 dark:bg-white/10" />
              </div>
            </>
          )}
