...
### Task Acceptance
1. Read `tasks/assignments/implementer-3.json` to get assigned task
...
2. Update `tasks/assignments/implementer-3.json` to idle status
3. Update heartbeat one final time

## Heartbeat Protocol
Update `status/agents/implementer-3.json` every 2 minutes with:
- Current task ID
