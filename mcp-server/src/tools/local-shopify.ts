...
function getShopifyClient(config: LocalConfig): ShopifyAPIClient {
  if (!config.store) throw new Error('No store configured. Set store in .synapse-theme/config.json or SHOPIFY_STORE env var.');

  const envToken = process.env.SHOPIFY_ACCESS_TOKEN;
  if (envToken) return new ShopifyAPIClient(config.store, envToken);

  const creds = loadCredentials(config.store);
  if (!creds) throw new Error(`No credentials found for store "${config.store}". Run synapse-theme auth first, or set SHOPIFY_ACCESS_TOKEN env var.`);

  return new ShopifyAPIClient(config.store, creds.accessToken);
}

// --- Theme intelligence helpers (regex-based, replaced by @synapse/theme-intelligence later) ---
...
function checkThemeFiles(workspacePath: string, filterFiles?: string[]): ThemeCheckIssue[] {
...
function checkLiquid(file: { path: string; content: string }, issues: ThemeCheckIssue[]): void {
...
function checkJSON(file: { path: string; content: string }, issues: ThemeCheckIssue[]): void {
  try {
  ...
  if (file.path.startsWith('sections/') && file.path.endsWith('.liquid')) return;
...
function traceRenderingChain(workspacePath: string, pageType: string): string[] {
  const chain: string[] = [];
...
function traceLiquidReferences(
  ...
  while ((match = renderRegex.exec(content)) !== null) {
    ...
    if (visited.has(snippetPath)) continue;
...
function findSettingUsage(workspacePath: string, settingId: string): { definedIn: string[]; usedIn: string[] } {
  ...
  for (const file of files) {
    ...
    if (file.path.endsWith('.liquid')) {
      ...
      if (schemaMatch) {
        try {
          ...
          const settings: Array<{ id?: string }> = schema.settings ?? [];
          if (settings.some((s) => s.id === settingId)) {
            definedIn.push(file.path);
          }
          // Also check block-level settings
          const blocks: Array<{ settings?: Array<{ id?: string }> }> = schema.blocks ?? [];
    ...
    if (file.path === 'config/settings_schema.json') {
      try {
        const schemas: Array<{ settings?: Array<{ id?: string }> }> = JSON.parse(file.content);
...
function findReferences(workspacePath: string, filePath: string): { referencedBy: string[]; references: string[] } {
  const files = readThemeFiles(workspacePath);
...
export function registerLocalShopifyTools(registry: ToolRegistry, config: LocalConfig): void {
  ...
  registry.register({
    ...
    async handler(args) {
      try {
        const settingId = args.settingId as string;
  ...
  registry.register({
    ...
    async handler(args) {
      try {
        const storeOverride = args.store as string | undefined;
  ...
  registry.register({
    ...
    async handler(args) {
      try {
        ...
        const content = await client.getAsset(args.themeId as string, args.key as string);
  ...
  registry.register({
    ...
    async handler(args) {
      try {
        const client = getShopifyClient(config);
        const resources = await client.listResources(args.resourceType as string);
